...
`;
    ...
    for (let j of responseJSON) {
      const symbolResult = await this.symbolGopls(searchFolderPath + j);
...
`;
  ...
  async symbolGopls(filePath: string): Promise<Symbol[]> {
    const {result, content} = await this.doSymbolGopls(filePath);
